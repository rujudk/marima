##'
##' @title Fit multivariate arima model using Spliid's algorithm.
##' 
...
##' procedure. Default is weight=0.50 which often works well. If weight>0.5
##' is specified more damping will result (e.g. weight=0.75).
...
##' # lost by differensing being excluded.
##' #
...
marima <-
 function(DATA=NULL,
          ...
          penalty=0.00,
          weight=0.33,
          Plot=FALSE,  #alternatives "trace" and "log.det"
          Check=FALSE)
{
 ...
 ar.pattern=array(c(diag(kvar),0*diag(kvar)),dim=c(kvar,kvar,2))}
     if(is.null(ma.pattern)){
 ma.pattern=array(c(diag(kvar),0*diag(kvar)),dim=c(kvar,kvar,2))
 max.iter <- 3
 till     <- 1
 weight   <- 0.0 }
...
if(length(Leftout)<=0){cat("All cases in data, ", min(case)," to ", max(case),
...
xmean<-means
if(length(xmean)==1) {if(xmean==1){means<-rep(1,kvar)}}
if(length(xmean)==1) {if(xmean!=1){means<-rep(0,kvar)}}

if(Check){
cat("Control printout (Check=TRUE (default)). \n")
cat("----------------------\n")
cat("Calling marima. Data dimensions (kvar,N) = ",kvar,N,"\n")
cat("Coefficient polynomial dimensions = ",dim(AR)," and ",dim(MA),"\n")

if(is.null(ar.pattern)){cat("no ar.pattern specified \n")}
if(is.null(ma.pattern)){cat("no ma.pattern specified \n")}
...
cat("max.iter=",max.iter,", means=",means,", penalty=",penalty,"\n")
cat("weight="  ,weight,  ", Plot= ",Plot, ", Check  =",Check  ,"\n")
...
On<-rep(0,kvar)
for (i in 1:kvar){ if(sum(AR[i,,])>1){On[i]<-i }
if(sum(AR[,i,])>1){On[i]<-i}}
On<-On[On>0]
cases1<-which(complete.cases(DATA[,On]))
rownames(DATA)<-as.character(1:dim(DATA)[1])
DATA<-DATA[cases1,]
D<-dim(DATA)
X<-lagged.data(DATA=DATA,AR=AR,MA=MA,init=TRUE,means=xmean)
averages<-X$averages
...
stt<-ARlags*kvar+1 ; stp<-ARlags*kvar+kvar
for(i in 1:length(stt)){
    if(i==1){arcols<-c(stt[i]:stp[i])}
    if(i >1){arcols<-c(arcols,c(stt[i]:stp[i]))}}

stt<-MAlags*kvar+1 ; stp<-MAlags*kvar+kvar
for(i in 1:length(stt)){
    if(i==1){macols<-c(stt[i]:stp[i])}
    if(i >1){macols<-c(macols,c(stt[i]:stp[i]))}}

ARDATA <- fill.out(DATAarray=array(X$ardata,dim=c(N,kvar,L)),SAR=SAR)
MADATA <- fill.out(DATAarray=array(X$madata,dim=c(N,kvar,M)),SAR=SAM)
...
if(L>1){
    for(i in 2:L){On2[,i]<-On2[,1]+(i-1)*kvar}}
cases<-which(complete.cases(ARDATA[,c(On2)]))

ARDATA<-ARDATA[cases,]
MADATA<-MADATA[cases,]
N<-length(cases)
# cat("rownames ARDATA", rownames(ARDATA), "\n")
# cat("rownames MADATA", rownames(MADATA), "\n")
...
if(iterate==max.iter){
if(i==1){ar.estimates<-matrix(AR*0,nrow=kvar)
...
MADATA <- fill.out(DATAarray=array(MADATA,dim=c(N,kvar,M)),SAR=SAM)
MADATA <- matrix(MADATA,nrow=N) 
...
# cat("Dimensions are:",dim(ARDATA[,1:kvar])," og ",dim(MADATA[,1:kvar]),"\n")
...
if(length(varx)>0){formula<-formul}
return(formula) }

lm.form <- function(y,arnames,zero){
if ( zero) {formula <- paste(arnames[y]," ~ -1 ",sep="")}
if (!zero) {formula <- paste(arnames[y]," ~    ",sep="")}
return(formula) }

lagged.data <- function(DATA=NULL,AR=NULL,MA=NULL,init=FALSE,means=1)
{
xmean<-means
N<-dim(DATA)[1]
kvar<-dim(DATA)[2]
...
for (i in 1:kvar){
    ...
    if(means[i]==1){DATA[,i]<-DATA[,i]-averages[i]}
...
# print(AR)
      ...
      vars<-1:kvar
#      cat("variables:",vars,"\n")
for (i in 1:kvar){if(sum(AR[i,,])==1){vars[i]=0}}
for (i in 1:kvar){
    S<-sum(AR[i,,])
    E<-rep(0,N)
    if(S>1){
...
#  cat("X",X,"\n")
#  cat("i=",i,"  Extra=",X,"\n")
    E<-arima(ardata[,i],order=c(3,0,0),xreg=ardata[,X])$residuals
    madata[,i]<-E}}}
...
return(list(ardata=ardata,madata=madata,averages=averages))}
