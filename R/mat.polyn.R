##'
##' @title Calculation of left inverse of matrix polynomial Leading term
##' is expected to be the (k by k) identity matrix. Is checked.
##' 
##' phi = matrix polynomial coefficients = I,phi1,phi2,...phi(ip)
##' dim(phi) = c(k,k,ip+1)
...
##' @title Calculation of product of two matrix polynomials (arrays).
##' 
##' If one or both leading unity matrices missing, they are (it is)
...
pol.mul <- function(eta,theta,L){
   ...
   if (L<1){L<-dim(eta)[3]+dim(theta)[3]-1}
   L<-L+1
  k   <- dim(eta)[1]
  iet <- dim(eta)[3]
  ith <- dim(theta)[3]
...
##'
##' @title Calculation of random shock form for arma model
##'
...
##' @title Calculation of inverse form for arma model
##' 
##' @param ar = autoregressive matrix part of model (array(k,k,ar-order)).
##' @param ma = moving average matrix part of model (array(k,k,ma-order)).
...
##'
##' @title Function to check and insert leading unity matrix if NOT present 
...
if(is.null(polyn)){stop("No input polynomial to 'check.one'. \n")}
  d<-dim(polyn)[1]
  dd<-dim(polyn)
  if(is.null(d)){cat("Empty polynomial in call to check.one \n")}
  if(length(dim(polyn))<2)
  {cat("Argument not polynomial in call to check.one")
   cat(dd," ",c(polyn),"\n")}
  pol2 <- polyn
  if(length(dim(polyn))<3){pol2<-array(polyn,dim=c(d,d,1))}
  if (max(abs(pol2[,,1]-diag(1,d)))>1.e-16){
    pol2<-lead.one(pol2,+1)}
    pol2[,,1]<-diag(1,d)      # set leading polyn = unity exactly.
return(pol2) }

#
# title: Function to add or remove leading identity matrix from
# matrix polynomial. Is used by function 'check.one'.
#
# polyn =  matrix polynomial = array of ar- or ma-coefficients.
# add   = +1 or -1 for addition or removal of leading unity matrix.
# return      = polyn with or without a leading unity matrix. 
#

lead.one <- function(polyn,add=0) {
  d<-dim(polyn)[1]
  dd<-dim(polyn)
  if(is.null(d)){cat("Empty polynomial in call to lead.one,\n")}
  if(length(dim(polyn))<2)
  {cat("Argument not polynomial in call to lead.one");cat(dd,c(polyn),"\n")}
  l  <- dim(polyn)[3]
  d  <- dim(polyn)[1]
  if (add == 1)     {
...
##'
##' @title Function to evaluate (significant)
