##'
##' @title Function to define multivariate arma model
##' for marima.
##' 
...
##' @param reg.var = no. of variable(s) that can only act as regression variable
##' such as (typically) a socalled leading indicator.
...
##'
 ...
 define.model <- function(kvar=2,ar=2,ma=1,rem.var=0,reg.var=0,
                          no.dep=NULL,print=0,
            ar.fill=NULL,ar.rem=NULL,ma.fill=NULL,ma.rem=NULL,indep=NULL){
if(print!=0){cat("Call = kvar,ar,ma,rem.var,reg.var,no.dep  \n",
  "kvar",kvar,"ar",ar,"ma",ma,"rem.var",rem.var,"reg.var",reg.var,"\n
   no.dep=",no.dep , "\n")}
...
L<-max(ar)+max(ma)
M<-1
if(max(ar)>=0){M<-max(ar)+1}

if((max(ar)+max(ma))<=0){
    cat("No model specified in function season.model! Returning. \n")
    return(pattern=NULL,indic=NULL)}
...
if(print!=0){cat("ind.poly=",ind.poly,"\n")
             cat("rem.var=" ,rem.var ,"\n") 
             cat("reg.var=" ,reg.var ,"\n")}
...
if(length(reg.var)>0){
if(print!=0){cat("ind.poly",ind.poly[reg.var,,],"\n")}
  ind.poly[reg.var,,] <- 0
...
if(print!=0){ cat("Model=(kvar,( ar,ma ),M,L,reg.var,rem.var,
(dim(ind.poly)) \n",
   kvar,"(",c(ar),c(ma),")",M,L,reg.var,rem.var,"(",dim(ind.poly),") \n")}
...
for (i in 1:kvar){
  if (i==1){indic<-c(ind.poly[i,,])}
  if (i>1 ){indic<-c(indic,c(ind.poly[i,,]))}}
...
poly<-array(ind.poly,dim=c(kvar,kvar,(max(ar)+max(ma))))
  ...
  ma.poly<-ar.poly
if(max(ar)>0){ar.poly<-check.one(poly[,,1:max(ar)])}
if(max(ma)>0){ma.poly<-check.one(poly[,,M:L])}
...
if (!is.null(no.dep)){ # cat("no.dep = ",no.dep,"\n")
no.dep<-matrix(no.dep,nrow=2)
if(print!=0){ cat("no.dep=",no.dep, "\n")}
  L<-dim(no.dep)[2]
 #  cat("L=",L,"\n")
  for (i in 1:L) {
 #  cat("i=",i,"L=",L)
...
for (j in 1:L){ar.poly[no.dep[1,j],no.dep[2,j],1:da]<-0}
for (j in 1:L){ma.poly[no.dep[1,j],no.dep[2,j],1:dm]<-0}
}}

if (!is.null(ar.fill)){
  arfill<-matrix(ar.fill,nrow=3)
if(length(c(arfill))!=length(c(ar.fill))){cat(ar.fill,"ar.fill error \n")}
  L<-dim(arfill)[2]
  for (j in 1:L){
...
if(length(c(arrem))!=length(c(ar.rem))){cat(ar.rem,"ar.rem error \n")}
  L<-dim(arrem)[2]
  for (j in 1:L){
...
if(length(c(mafill))!=length(c(ma.fill))){cat(ma.fill,"ma.fill error \n")}
  L<-dim(mafill)[2]
  for (j in 1:L){
...
if(length(c(marem))!=length(c(ma.rem))){cat(ma.rem,"ma.rem error \n")}  
  L<-dim(marem)[2]
  for (j in 1:L){
...
##' @title Function to define multivariate arma model (ar-polynomium and
##' ma-polynomium). The routine is used by marima only and not intended
##' the user (you!).

indicator <- function(kvar=2,ar.poly=array(diag(kvar),dim=c(kvar,kvar,1)),
                             ma.poly=array(diag(kvar),dim=c(kvar,kvar,1)))
# generates indicator array for defining arma model.
...
mar<-dim(ma.poly)[3]
L<-lar+mar-2
indic<-matrix(0,kvar,(kvar*L))

 if(L<=0){
    cat("Empty or no model in call to routine 'indicator', returning! \n")
    return(indic)}

for (i in 1:kvar){
if(lar>=2){if(mar>=2)  {
   indic[i,1:(kvar*L)]<-c(c(ar.poly[i,,2:lar]),c(ma.poly[i,,2:mar]))}}
if(lar>=2){if(mar<2)   {
   indic[i,1:(kvar*L)]<-c(c(ar.poly[i,,2:lar]))}}
if(lar<2){if(mar>=2)   {
   indic[i,1:(kvar*L)]<-c(c(ma.poly[i,,2:mar]))}}  }
   indic<-t(indic)

return(indic) }

##'
##' @title Function to print out matrix polynomium leaving out
##' empty lag matrices and, if specified, the leading (unity)
##' matrix.
##' 
##' @param poly = matrix polynomium (as construced by define.model,
##' for example, or estimated by marima).
##' @param name = character string used as header in output (default="lag").
##' @param leading = TRUE/FALSE. If FALSE leading (unity matrix) is to be
##' suppressed.
##'
##' @examples
...
short.form <- function(poly=NULL,name="Lag=",leading=TRUE,tail=FALSE) {
...
pol.names  <- paste(name, 0:(LL+1) , sep="" )
...
# cat("Lags=",Lags,"\n")
# print(poly)
if(leading!=TRUE & max(Lags)>1){Lags<-setdiff(Lags,1)}

Poly <- round(poly[,,Lags],6)
# cat("Poly=","\n")
# print(Poly)
# cat("Lags=",Lags,"\n")
return(Poly)}
...
# ######## old version of short.form #########


short.form.old <- function(poly=NULL,name="Lag=",leading=TRUE) {

poly <- check.one(poly)
kvar<-dim(poly)[1]
L<-dim(poly)[3]

pol.names  <- paste(name,(0:L)  ,sep="" )
out.names  <- paste("y" ,c(1:kvar),sep="" )
inpx.names <- paste("x",c(1:kvar),sep="" )
inpy.names <- paste("=y",c(1:kvar),sep="" )
inpy.names <- paste(inpx.names,inpy.names,sep="")
dimnames(poly) <- list(out.names,inpy.names,pol.names[1:L])

pol.act   <- sort(which(colSums(poly^2,dims=2)!=0))
Poly <- round(poly[,,pol.act],4)
# Poly is an array

LL<-dim(Poly)
if(length(LL)==2){Poly<-array(Poly,dim=c(LL,1))}
# cat("Down in short.form",dim(Poly),"\n")
LL<-dim(Poly)[3]
if(LL>=2){
if(leading==FALSE){Poly<-Poly[,,2:LL]}}
if(LL==1){{Poly<-Poly[,,1]}}
return(Poly)}

##'
##' @title Function to generate and apply a differencing 
##' matrix polynomial (autoregressive form) defined by a pattern.
##' 
...
d<-dim(series)
T<-0
if(d[1]>d[2]){series<-t(series);T<-1;Names<-rownames(series)}
d<-dim(series)
kvar<-d[1]
OMIT<-0
DM<-rep(0,kvar)
...
for (i in 1:LL){
    if(sum(colSums(matrix(pol[,,i])))!=0) J<-i}
dif.poly<-pol[,,(1:J)] }
...
for (i in 1:length(vars)){
    if (vars[i]==1) {dif.series[i,]<-dif.series[i,]+Means[i]}}
# cat("means=",Means)
# cat("vars =",vars)

#
# if(omit!=TRUE){cat("All data used. Differencing with respect to previous",
# "values \n (negative time index) was done relative to the mean of
# the series.\n")} 
...
return(list(y.dif=y.dif,y.lost=y.lost,dif.poly=dif.poly,
averages=Means,dif.series=dif.series)) }
...
# Function to do differencing of univariate time series
# (inverse function of ser.sum(...)).
# param y = (univariate) time series
# param s = time difference for differencing (default s=1)
# examples y.dif <- ser.dif(y,s=1)
#
# Y<-rnorm(100); dim(Y)<-c(50,2);
# plot(Y[,2],series.sum(series.dif(Y[,2],s=7),s=7))
# 

ser.dif <- function(y,s=1)
{   # y = time series (univariate)
    # s = differencing for time=s 
       z<-y
    for (i in 1:length(y)){
        j<-i-s
        if(j>0){z[i]<-y[i]-y[j]}}
...
##'
##' @title Function to aggregate multivariate time series.
##' Reverse of function 'define.dif'.
##' @param series = series that have to be summed up.
...
for (i in 1:d[1]){series[i,]<-series[i,]+averages[i]}}
return(list(series.sum=series)) }

#
# Function to do summation of univariate time series
# (inverse function of series.dif(...)).
# param y = (univariate) time series
# param s = time difference for summation 
# examples y.sum <- ser.sum(y,s=1)
# 

ser.sum <- function(y,s=1)    
{   # y = time series (univariate)
    # s = summation for time difference=s (default s=1)
    z<-y
    for (i in 1:length(y)){
        j<-i-s
        if(j>0){z[i]<-z[i]+z[j]}}
...
##'
##' @title Generate new time series with (seasonally) lagged variables
...
L<-max(lagging[3,])
  if(L>=s){stop("Returning from function saeson.lagging: \n
       max lagging",L,
...
rownames(newy)<-c(c(paste("y",c(1:k),sep="")),
     c(paste("y",c(lagging[1,]),"-",c(lagging[3,]),sep="")))
...
return( list(y.lagged=y.lagged, y.future=y.future, y.lost=y.lost))
}
